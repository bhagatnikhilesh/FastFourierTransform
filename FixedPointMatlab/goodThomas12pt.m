...
N = 12;     % input('Enter the number of points: ');
N1 = 4;     % columns
N2 = 3;     % rows
...
for row = 1:N1:N
    [DataMemR(row:row+N1-1) DataMemJ(row:row+N1-1)] = wfta4ptFunction(DataMemR7in(row:row+N1-1), DataMemJ7in(row:row+N1-1), WL ,WF, RoundMode, Signed);
end
