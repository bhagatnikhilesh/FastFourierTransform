...
N = 24;     % input('Enter the number of points: ');
N1 = 8;     % columns
N2 = 3;     % rows
...
%% =================================================================
%                    CRT MAPPING
CRTindexValue63ptDouble = CRTmemoryModuleFunctionTrans(initCRT, N1, N2); 
% for i = 1:1:N
...
for row = 1:N1:N
    [DataMemR(row:row+N1-1) DataMemJ(row:row+N1-1)] = wfta8ptFunction(DataMemR7in(row:row+N1-1), DataMemJ7in(row:row+N1-1), WL ,WF, RoundMode, Signed);
end
...
for row = 1:1:N1
    ...
    [OutN2ptFFTR OutN2ptFFTJ] = wfta3ptFunction(InN2ptFFTR, InN2ptFFTJ, WL ,WF, RoundMode, Signed);
    for col = 1:1:N2
